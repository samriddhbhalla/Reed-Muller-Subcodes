% k=sum of binomial;
% (sum of bin till R) > k;
function [B, Code]=rmDecode_v2(EncWord,G,R,M,k,y)

%% if we use the kroneker product wala G;
%  G=sorter(G,M);
%  G=G(1:k, :);
% G=reedmullergen(R,M);
[nosWords,~]=size(EncWord);%rows and columns
...
%% incase z==length(y) ie k=is sum of binomial oooh same thing so z==k
if(z==k)
    for idx=1:nosWords
   b=EncWord(idx, :);%Current encoded word to be decoded
...
elseif(z~=k)
   ...
   lastLength=size(Pi{y(k)},2);%length(Pi{k});
   for step=k:-1:1
        %whenever there is a change in length of Pi ie a change in order
        %what we do is that we update the received vector. also in case R=1
        %it means we have come to the final step where we need to do the
        ...
        %upon which we will do all the above operations
        modRec=[];
        if(lastLength~=size(Pi{y(step)},2) || y(step)==1)
            modRec=mod(Cd(step+1: end)*GCopy(step+1: end, :),2);%basically reencode the last lines and delete the copy slowly in R+1 steps
            b=mod(b+modRec,2);
            GCopy(step+1:end, :)=0;
            Degree=Degree-1;
            if(y(step)==1)
                Cd(y(step))=majorty_logic_check(b);
                break;
               %put parity check case here; 
            end
        end
        lastLength=size(Pi{y(step)},2);
        Si=Pi{y(step)};     %indices of set S 
        Si=sort(Si);
        ...
        U=1:M;  %Universal Set % E=U\pi{y(step)}
        Ei=setdiff(U,Si);
%         Ei=sort(Ei-1);  %this y(step) is just for clarity will be removed later
        if(size(Ei,2)~=0)   %incase R=M;        
            perm_Dij=de2bi(0:2.^(M-Degree) -1,'left-msb');
            powOf2=2.^(Ei-1);
            powOf2=repmat(powOf2,2.^(M-Degree),1);
